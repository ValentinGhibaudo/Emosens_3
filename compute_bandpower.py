...
def compute_bandpower(run_key, **p):
    ...
    for chan in psd_eeg.coords['chan'].values:
        ...
        for band, bornes in p['fbands'].items():
            ...
            row = [participant, session, chan, band, power_mean, power_median, power_integral]
            rows.append(row)
    
    bandpowers = pd.DataFrame(rows, columns = ['participant','session','chan','band','power_mean','power_median','power_integral'])
